...
public class CarWashState extends SimState {
	...
	public void removeFromMachines(Car car) {
		...
		for (int i = 0; i < fastWashes.size(); i++) {
			if (fastWashes.get(i).hasCar(car)) {
				emptyMachines.add(0, fastWashes.get(i));
				fastWashes.get(i).removeCar();
				info.emptyFast++;
			}
		}
		for (int i = 0; i < slowWashes.size(); i++) {
			if (slowWashes.get(i).hasCar(car)) {
				emptyMachines.add(slowWashes.get(i));
				slowWashes.get(i).removeCar();
				info.emptySlow++;
			}
		}
	}
